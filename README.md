...
## Важно: хостинг и проверка доступности

Проверка доступности выполняется в браузере через загрузку изображений (`<img>`) с проверяемых доменов. Если хостинг (или браузер) ограничивает загрузку внешних ресурсов (например, строгая Content-Security-Policy), проверка может не сработать и вы увидите «Контрольный example.com недоступен».

- **GitHub Pages** — добавлен мета-тег CSP `img-src 'self' https: data:`, но если GitHub отдаёт свою политику в заголовках, проверка может по-прежнему не работать. В этом случае разверните сайт на **Netlify**, **Cloudflare Pages** или откройте **локально** (см. ниже).
- **Netlify / Cloudflare Pages / свой сервер** — обычно подходят без ограничений; при необходимости настройте заголовки так, чтобы разрешить `img` с `https:`.
...
## Локальный запуск (рекомендуется, если на GitHub Pages проверка не работает)

Поднимите локальный HTTP-сервер, чтобы подгружались `data/` и не было ограничений на загрузку внешних изображений:
...
Либо разверните тот же каталог на **Netlify** или **Cloudflare Pages** (drag-and-drop или через Git) — проверка обычно работает без доп. настроек.
